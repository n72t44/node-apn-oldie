...
/**
 * Set the sound name for the notification - used with iOS Critical Alerts on iOS12 devices
 * @param {String} text The name of the sound file.
 * @since v1.7.8c
 */
Notification.prototype.setSoundName = function (text) {
	if(typeof this.sound !== "object") {
		this.sound = text;
	}
	else {
		this.prepareSound();
		this.sound.name = text;
	}
	return this;
};
...
/**
 * Set the sound volume for the notification - used with iOS Critical Alerts on iOS12 devices
 * @param {Number} soundVolume The volume for the alert.
 * @since v1.7.8c
 */
Notification.prototype.setSoundVolume = function(soundVolume) {
	this.prepareSound();
	this.sound.volume = soundVolume;
	return this;
};
/**
 * Set the critical flag for the notification - used with iOS Critical Alerts on iOS12 devices
 * @param {Boolean} isCritical The critical flag for the alert.
 * @since v1.7.8c
 */
Notification.prototype.setSoundCritical = function(isCritical) {
	this.prepareSound();
	this.sound.critical = isCritical ? 1 : 0;
	return this;
};
...
/**
 * If a sound object doesn't already exist create it and transfer any existing sound name into the .name property
 * @private
 * @since v1.7.8c
 */
Notification.prototype.prepareSound = function () {
	var existingValue = this.sound;
	if(typeof existingValue !== "object") {
		this.sound = {};
		if(typeof existingValue === "string") {
			this.sound.name = existingValue;
		}
	}
};
